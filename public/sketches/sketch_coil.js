...
function setup() {
  ...
  typeXSlider = createSlider(0, 100, 7);
  typeXSlider.position(25, 20);
  typeXSlider.style('width', '100px');
  typeYSlider = createSlider(0, 100, 20);
  typeYSlider.position(25, 50);
  typeYSlider.style('width', '100px');
  typeStrokeSlider = createSlider(0, 5, 2, 0.5);
  typeStrokeSlider.position(25, 80);
  typeStrokeSlider.style('width', '100px');

  ribbonCountSlider = createSlider(0, 400, 40);
  ribbonCountSlider.position(25, 130);
  ribbonCountSlider.style('width', '100px');
  ribbonSizeSlider = createSlider(-10, 50, 10);
  ribbonSizeSlider.position(25, 160);
  ribbonSizeSlider.style('width', '100px');

  radioEnd = createRadio();
  radioEnd.position(35, 195);
  radioEnd.style('width', '30px');
  radioEnd.option(' ', 1);
  radioEnd.option(' ', 2);
  radioEnd.value('1').checked = true;

  radiusSlider = createSlider(0, 25, 3, 0.5);
  radiusSlider.position(25, 260);
  radiusSlider.style('width', '100px');
  trackerSlider = createSlider(0, 50, 10);
  trackerSlider.position(25, 290);
  trackerSlider.style('width', '100px');
  spiralStartSlider = createSlider(0, 200, 50);
  spiralStartSlider.position(25, 320);
  spiralStartSlider.style('width', '100px');
  spinSlider = createSlider(0, 10, 1);
  spinSlider.position(25, 350);
  spinSlider.style('width', '100px');

  waveSizeSlider = createSlider(0, 200, 0);
  waveSizeSlider.position(25, 410);
  waveSizeSlider.style('width', '100px');
  waveCountSlider = createSlider(0, 12, 1);
  waveCountSlider.position(25, 440);
  waveCountSlider.style('width', '100px');
  waveSpeedSlider = createSlider(0, 10, 1);
  waveSpeedSlider.position(25, 470);
  waveSpeedSlider.style('width', '100px');
  slopeSlider = createSlider(0, PI, 1, 0.1);
  slopeSlider.position(25, 500);
  slopeSlider.style('width', '100px');

  //  resetSet = createButton('Reset'); resetSet.position(25,height-60); resetSet.mousePressed(reset);
  wideSet = createButton('Wide');
  wideSet.position(25, height - 60);
  wideSet.mousePressed(wide);
  superSet = createButton('Super');
  superSet.position(75, height - 60);
  superSet.mousePressed(tall);
  amoebaSet = createButton('Amoeba');
  amoebaSet.position(130, height - 60);
  amoebaSet.mousePressed(amoeba);
  spacerSet = createButton('Spacer');
  spacerSet.position(200, height - 60);
  spacerSet.mousePressed(spacer);
  kittySet = createButton('Kitty');
  kittySet.position(260, height - 60);
  kittySet.mousePressed(kitty);

  hourglassSet = createButton('Hourglass');
  hourglassSet.position(25, height - 35);
  hourglassSet.mousePressed(hourglass);
  starSet = createButton('Star');
  starSet.position(100, height - 35);
  starSet.mousePressed(star);
  star2Set = createButton('ZZtar');
  star2Set.position(145, height - 35);
  star2Set.mousePressed(star2);
  pretzelSet = createButton('Pretzel');
  pretzelSet.position(200, height - 35);
  pretzelSet.mousePressed(pretzel);
  lemniscateSet = createButton('Lemniscate');
  lemniscateSet.position(260, height - 35);
  lemniscateSet.mousePressed(lemniscate);
  ...
  bkgdColorPicker.position(25, 580);
...
function draw() {
  ...
  for (var k = 0; k < ribbonCount; k++) {
    ...
    if (inp0check.checked() == false) {
      ...
      beginShape();
      for (
        var i = spiralStart + inpText.length * k - 1;
        i <= spiralStart + inpText.length + inpText.length * k;
        i++
      ) {
        var polarX =
          -radiusAdjusted * sqrt(tracking * i) * cos(sqrt(tracking * i));
        var polarY =
          -radiusAdjusted * sqrt(tracking * i) * sin(sqrt(tracking * i));

        var echo = atan2(polarY, polarX);
        var theWave =
          sinEngine(echo, waveCount, waveSpeed / 100, slope) * waveSize;
    ...
    strokeJoin(ROUND);
    for (
      var i = spiralStart + inpText.length * k;
      i < spiralStart + inpText.length + inpText.length * k;
      i++
    ) {
      ...
      var preX =
        -radiusAdjusted *
        sqrt(tracking * (i - 1)) *
        cos(sqrt(tracking * (i - 1)));
      var preY =
        -radiusAdjusted *
        sqrt(tracking * (i - 1)) *
        sin(sqrt(tracking * (i - 1)));
      var postX =
        -radiusAdjusted *
        sqrt(tracking * (i + 1)) *
        cos(sqrt(tracking * (i + 1)));
      ...
      var preWave =
        sinEngine(preEcho, waveCount, waveSpeed / 100, slope) * waveSize;
      ...
      var postWave =
        sinEngine(postEcho, waveCount, waveSpeed / 100, slope) * waveSize;
      ...
      var delta = atan2(
        postY + postWaveY - (preY + preWaveY),
        postX + postWaveX - (preX + preWaveX)
      );
...
function reset() {
  ...
  ribbonCountSlider.value(40);
  ribbonSizeSlider.value(10);
  radiusSlider.value(3);
  trackerSlider.value(10);
  spiralStartSlider.value(50);
  spinSlider.value(1);

  waveSizeSlider.value(0);
  waveCountSlider.value(2);
...
function wide() {
  reset();
  typeXSlider.value(18);
  typeYSlider.value(10);
  typeStrokeSlider.value(2);
  ribbonCountSlider.value(100);
  ribbonSizeSlider.value(15);
  radiusSlider.value(15);
  trackerSlider.value(2);
  spiralStartSlider.value(40);
  spinSlider.value(1);
...
function tall() {
  ...
  typeYSlider.value(68);
  typeStrokeSlider.value(1.5);
  ribbonCountSlider.value(40);
  ribbonSizeSlider.value(25);
  radiusSlider.value(1);
  trackerSlider.value(4);
  spiralStartSlider.value(6);
  spinSlider.value(1);
...
function amoeba() {
  reset();
  typeXSlider.value(9);
  typeYSlider.value(14);
  typeStrokeSlider.value(1.5);
  ribbonCountSlider.value(25);
  ribbonSizeSlider.value(22);
  radiusSlider.value(6);
  trackerSlider.value(10);
  spiralStartSlider.value(32);
  spinSlider.value(1);

  waveSizeSlider.value(100);
  waveCountSlider.value(2);
  waveSpeedSlider.value(1);
  slopeSlider.value(1);
  radioEnd.value('2').checked = true;
...
function star() {
  ...
  typeYSlider.value(20);
  typeStrokeSlider.value(2);
  ribbonCountSlider.value(230);
  ribbonSizeSlider.value(0);
  radiusSlider.value(4.5);
  trackerSlider.value(0);
  spiralStartSlider.value(122);
  spinSlider.value(2);

  waveSizeSlider.value(25);
  waveCountSlider.value(5);
  waveSpeedSlider.value(0);
  slopeSlider.value(1);
  radioEnd.value('1').checked = true;
...
function spacer() {
  ...
  typeStrokeSlider.value(2.5);
  ribbonCountSlider.value(110);
  ribbonSizeSlider.value(-10);
  radiusSlider.value(3.5);
  trackerSlider.value(1);
  spiralStartSlider.value(22);
  spinSlider.value(1);

  waveSizeSlider.value(0);
  waveCountSlider.value(0);
  waveSpeedSlider.value(0);
  slopeSlider.value(0);
  radioEnd.value('1').checked = true;
...
function kitty() {
  reset();
  typeXSlider.value(6);
  typeYSlider.value(23);
  typeStrokeSlider.value(1.5);
  ribbonCountSlider.value(217);
  ribbonSizeSlider.value(9);
  radiusSlider.value(1.5);
  trackerSlider.value(5);
  spiralStartSlider.value(5);
  spinSlider.value(0);
...
function hourglass() {
  reset();
  typeXSlider.value(2);
  typeYSlider.value(36);
  typeStrokeSlider.value(2);
  ribbonCountSlider.value(337);
  ribbonSizeSlider.value(30);
  radiusSlider.value(3);
  trackerSlider.value(3);
  spiralStartSlider.value(96);
  spinSlider.value(1);

  waveSizeSlider.value(83);
  waveCountSlider.value(2);
...
function star2() {
  ...
  typeStrokeSlider.value(2);
  ribbonCountSlider.value(400);
  ribbonSizeSlider.value(43);
  radiusSlider.value(12.5);
  trackerSlider.value(0);
  spiralStartSlider.value(30);
  spinSlider.value(1);

  waveSizeSlider.value(31);
  waveCountSlider.value(6);
  waveSpeedSlider.value(7);
  slopeSlider.value(1);
  radioEnd.value('1').checked = true;
...
function pretzel() {
  ...
  typeStrokeSlider.value(2);
  ribbonCountSlider.value(400);
  ribbonSizeSlider.value(31);
  radiusSlider.value(3);
  trackerSlider.value(1);
  spiralStartSlider.value(0);
  spinSlider.value(1);

  waveSizeSlider.value(200);
  waveCountSlider.value(1);
...
function lemniscate() {
  ...
  typeStrokeSlider.value(2);
  ribbonCountSlider.value(400);
  ribbonSizeSlider.value(50);
  radiusSlider.value(0);
  trackerSlider.value(21);
  spiralStartSlider.value(84);
  spinSlider.value(1);
