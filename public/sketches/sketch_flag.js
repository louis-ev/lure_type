...
function setup() {
  ...
  typeXSlider = createSlider(0, 150, 20);
  typeXSlider.position(15, 30);
  typeXSlider.style('width', '100px');
  typeYSlider = createSlider(0, 150, 50);
  typeYSlider.position(15, 60);
  typeYSlider.style('width', '100px');
  typeStrokeSlider = createSlider(0, 4, 2, 0.25);
  typeStrokeSlider.position(15, 90);
  typeStrokeSlider.style('width', '100px');
  rowSlider = createSlider(0, 30, 6);
  rowSlider.position(15, 120);
  rowSlider.style('width', '100px');
  paddingSlider = createSlider(0, 100, 30);
  paddingSlider.position(15, 150);
  paddingSlider.style('width', '100px');

  xWaveSlider = createSlider(0, 200, 95);
  xWaveSlider.position(15, 210);
  xWaveSlider.style('width', '100px');
  yWaveSlider = createSlider(0, 200, 20);
  yWaveSlider.position(15, 240);
  yWaveSlider.style('width', '100px');
  zWaveSlider = createSlider(0, 200, 50);
  zWaveSlider.position(15, 270);
  zWaveSlider.style('width', '100px');
  offsetSlider = createSlider(0, PI / 4, 0.3, 0.01);
  offsetSlider.position(15, 300);
  offsetSlider.style('width', '100px');
  speedSlider = createSlider(0, 0.5, 0.03, 0.01);
  speedSlider.position(15, 330);
  speedSlider.style('width', '100px');
  rowOffsetSlider = createSlider(0, PI, 0.37, 0.01);
  rowOffsetSlider.position(15, 360);
  rowOffsetSlider.style('width', '100px');
  slopeSlider = createSlider(0, PI, 1, 0.1);
  slopeSlider.position(15, 390);
  slopeSlider.style('width', '100px');

  typePushSlider = createSlider(0, 10, 2);
  typePushSlider.position(15, 510);
  typePushSlider.style('width', '100px');
  ...
  oneBannerSet = createButton('A Banner');
  oneBannerSet.position(150, height - 60);
  oneBannerSet.mousePressed(oneBanner);
  twistSet = createButton('A Twist');
  twistSet.position(220, height - 60);
  twistSet.mousePressed(twist);
  foldsSet = createButton('Folds');
  foldsSet.position(280, height - 60);
  foldsSet.mousePressed(folds);
  flatSeaSet = createButton('Flat Sea');
  flatSeaSet.position(330, height - 60);
  flatSeaSet.mousePressed(flatSea);
  barberSet = createButton('Barber');
  barberSet.position(395, height - 60);
  barberSet.mousePressed(barber);
  siloSet = createButton('Silos');
  siloSet.position(450, height - 60);
  siloSet.mousePressed(silos);
  mysterySet = createButton('Mystery');
  mysterySet.position(500, height - 60);
  mysterySet.mousePressed(mystery);

  colaWaveSet = createButton('Cola Waves');
  colaWaveSet.position(150, height - 40);
  colaWaveSet.mousePressed(colaWave);
  origamiSet = createButton('Origami');
  origamiSet.position(230, height - 40);
  origamiSet.mousePressed(origami);
  craneSet = createButton('Origami 2');
  craneSet.position(293, height - 40);
  craneSet.mousePressed(crane);
  blackwhiteSet = createButton('B&W');
  blackwhiteSet.position(365, height - 40);
  blackwhiteSet.mousePressed(blackwhite);
  newsprintSet = createButton('Newsprint');
  newsprintSet.position(415, height - 40);
  newsprintSet.mousePressed(newsprint);
  edgeSet = createButton('Edge Case');
  edgeSet.position(490, height - 40);
  edgeSet.mousePressed(edge);

  inp0check = createCheckbox('', false);
  inp0check.position(160, 22);
  inp1 = createColorPicker('#FF6A55');
  inp1.position(180, 50);
  inp1.style('width', '20px');
  inp1check = createCheckbox('', true);
  inp1check.position(160, 52);
  inp2 = createColorPicker('#FEB2AD');
  inp2.position(180, 80);
  inp2.style('width', '20px');
  inp2check = createCheckbox('', true);
  inp2check.position(160, 82);
  inp3 = createColorPicker('#FFFFF0');
  inp3.position(180, 110);
  inp3.style('width', '20px');
  inp3check = createCheckbox('', false);
  inp3check.position(160, 112);
  inp4 = createColorPicker('#218DA6');
  inp4.position(180, 140);
  inp4.style('width', '20px');
  inp4check = createCheckbox('', false);
  inp4check.position(160, 142);
  inp5 = createColorPicker('#BAF2E8');
  inp5.position(180, 170);
  inp5.style('width', '20px');
  inp5check = createCheckbox('', false);
  inp5check.position(160, 172);
...
function draw() {
  ...
  for (var j = 0; j < rows; j++) {
    for (var i = 0; i < inpText.length; i++) {
      ...
      var yWaverTR =
        sinEngine(i + 1, offset, j, rowOffset, speed, slope) * yWave;
      var yWaverBR =
        sinEngine(i + 1, offset, j + 1, rowOffset, speed, slope) * yWave;
      var yWaverBL =
        sinEngine(i, offset, j + 1, rowOffset, speed, slope) * yWave;

      var xWaverTL = sinEngine(i, offset, j, rowOffset, speed, slope) * xWave;
      var xWaverTR =
        sinEngine(i + 1, offset, j, rowOffset, speed, slope) * xWave;
      var xWaverBR =
        sinEngine(i + 1, offset, j + 1, rowOffset, speed, slope) * xWave;
      var xWaverBL =
        sinEngine(i, offset, j + 1, rowOffset, speed, slope) * xWave;

      var zWaverTL = sinEngine(i, offset, j, rowOffset, speed, slope) * zWave;
      var zWaverTR =
        sinEngine(i + 1, offset, j, rowOffset, speed, slope) * zWave;
      var zWaverBR =
        sinEngine(i + 1, offset, j + 1, rowOffset, speed, slope) * zWave;
      var zWaverBL =
        sinEngine(i, offset, j + 1, rowOffset, speed, slope) * zWave;
...
function reset() {
  ...
  typeYSlider.value(50);
  typeStrokeSlider.value(1.5);
  rowSlider.value(1);
  paddingSlider.value(20);
  typePushSlider.value(2);

  zWaveSlider.value(30);
  xWaveSlider.value(30);
  yWaveSlider.value(30);
  offsetSlider.value(0.3);
  slopeSlider.value(1);
  speedSlider.value(0.02);
  rowOffsetSlider.value(0.3);

  xRotCameraSlider.value(30);
  yRotCameraSlider.value(30);
  zRotCameraSlider.value(-30);
...
function oneBanner() {
  ...
  rowSlider.value(1);
  paddingSlider.value(20);
  typePushSlider.value(0);

  zWaveSlider.value(70);
  xWaveSlider.value(0);
  yWaveSlider.value(0);
  offsetSlider.value(0.3);
  speedSlider.value(0.02);
  rowOffsetSlider.value(0);

  xRotCameraSlider.value(25);
  yRotCameraSlider.value(75);
  zRotCameraSlider.value(-10);
  zoomCameraSlider.value(-150);
...
function folds() {
  reset();
  typeXSlider.value(45);
  typeYSlider.value(70);
  typeStrokeSlider.value(3);
  rowSlider.value(8);
  paddingSlider.value(20);
  typePushSlider.value(8.5);

  zWaveSlider.value(90);
  xWaveSlider.value(0);
  yWaveSlider.value(0);
  offsetSlider.value(0.21);
  speedSlider.value(0.02);
  rowOffsetSlider.value(2.55);

  xRotCameraSlider.value(51);
  yRotCameraSlider.value(37);
  zRotCameraSlider.value(-28);
  zoomCameraSlider.value(-300);
...
function twist() {
  ...
  rowSlider.value(1);
  paddingSlider.value(70);
  typePushSlider.value(4);
  ...
  offsetSlider.value(0.3);
  speedSlider.value(0.02);
  rowOffsetSlider.value(3.14);

  xRotCameraSlider.value(-45);
  yRotCameraSlider.value(37);
  zRotCameraSlider.value(-15);
  zoomCameraSlider.value(-120);
...
function flatSea() {
  reset();
  typeXSlider.value(25);
  typeYSlider.value(45);
  typeStrokeSlider.value(2);
  rowSlider.value(12);
  paddingSlider.value(40);
  typePushSlider.value(4);

  zWaveSlider.value(75);
  xWaveSlider.value(0);
  yWaveSlider.value(200);
  offsetSlider.value(0.11);
  speedSlider.value(0.08);
  rowOffsetSlider.value(1.96);
...
function colaWave() {
  ...
  typeStrokeSlider.value(3);
  rowSlider.value(4);
  paddingSlider.value(43);
  typePushSlider.value(0);

  zWaveSlider.value(0);
  xWaveSlider.value(0);
  yWaveSlider.value(150);
  offsetSlider.value(0.19);
  speedSlider.value(0.02);
  rowOffsetSlider.value(0.28);
...
function origami() {
  reset();
  typeXSlider.value(87);
  typeYSlider.value(83);
  typeStrokeSlider.value(3);
  rowSlider.value(18);
  paddingSlider.value(43);
  typePushSlider.value(10);
  ...
  xRotCameraSlider.value(42);
  yRotCameraSlider.value(20);
  zRotCameraSlider.value(-23);
  zoomCameraSlider.value(-500);
...
function blackwhite() {
  reset();
  typeXSlider.value(17);
  typeYSlider.value(43);
  typeStrokeSlider.value(2);
  rowSlider.value(10);
...
function newsprint() {
  reset();
  typeXSlider.value(42);
  typeYSlider.value(48);
  typeStrokeSlider.value(2);
  rowSlider.value(16);
  paddingSlider.value(48);
  typePushSlider.value(4);

  zWaveSlider.value(83);
  xWaveSlider.value(200);
  yWaveSlider.value(200);
  slopeSlider.value(3.1);
  offsetSlider.value(0.11);
  speedSlider.value(0.08);
  rowOffsetSlider.value(1.96);
...
function silos() {
  ...
  rowSlider.value(7);
  paddingSlider.value(20);
  typePushSlider.value(8);

  zWaveSlider.value(200);
  xWaveSlider.value(0);
  yWaveSlider.value(0);
  slopeSlider.value(3.1);
  offsetSlider.value(0.37);
...
function crane() {
  ...
  typeYSlider.value(40);
  typeStrokeSlider.value(1.75);
  rowSlider.value(10);
  paddingSlider.value(40);
  typePushSlider.value(3);

  zWaveSlider.value(32);
  xWaveSlider.value(115);
  yWaveSlider.value(0);
  slopeSlider.value(0.8);
  offsetSlider.value(0.2);
  speedSlider.value(0.03);
  rowOffsetSlider.value(2.47);

  xRotCameraSlider.value(40);
  yRotCameraSlider.value(0);
  zRotCameraSlider.value(0);
  zoomCameraSlider.value(0);
...
function edge() {
  ...
  yWaveSlider.value(0);
  zWaveSlider.value(0);
  slopeSlider.value(0.5);
  offsetSlider.value(0.42);
  speedSlider.value(0.02);
  rowOffsetSlider.value(0.46);

  xRotCameraSlider.value(0);
  yRotCameraSlider.value(11);
  zRotCameraSlider.value(30);
  zoomCameraSlider.value(-500);
...
function mystery() {
  ...
  typeStrokeSlider.value(2);
  rowSlider.value(15);
  paddingSlider.value(27);
  typePushSlider.value(0);

  xWaveSlider.value(0);
  yWaveSlider.value(71);
  zWaveSlider.value(73);
  slopeSlider.value(1);
